...
const TARGETS: { target: string; os: string; arch: string; bin: string }[] = [
  { target: "bun-windows-x64", os: "windows", arch: "x64", bin: "aihub-auto.exe" },
  ...
  { target: "bun-windows-arm64", os: "windows", arch: "arm64", bin: "aihub-auto.exe" },
];
...
for (const t of TARGETS) {
  ...
  const proc = await $`bun build --compile --minify --target=${t.target} ${join(root, "apps/router/src/main.ts")} --outfile ${out}`
    .quiet()
    .nothrow();
  if (proc.exitCode !== 0) {
    const text = proc.stderr.toString();
    if (t.target === "bun-windows-arm64" && /unsupported|invalid target|Unsupported/i.test(text)) {
      console.warn(`⚠ 跳过 ${name}:当前 bun 版本不支持该目标(Windows ARM 用户可运行 x64 版)`);
