...
- **真反代**:客户端指向本地代理,有效上游 TTFT 默认作为先验;本地实测足够可信且更快时才覆盖,最近 3 小时成功率单独计入风险
...
- **双执行模式**:默认自动 Key 池按组单飞创建/安全回收 Key;硬无效闲置组可越过旧亲和强制回收,而当前/创建/预留/在飞组始终保护;single 兼容模式继续支持单 Key `PUT` 切组
