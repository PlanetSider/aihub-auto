...
- **真反代**:客户端指向本地代理,有效云端 TTFT 作为先验;有本地请求样本后按置信度双向融合,最近 3 小时成功率单独计入风险
...
- **双执行模式**:默认自动 Key 池按组单飞创建/安全回收 Key;24 小时会话映射只在短缓存窗口内保护 Key,超窗后 Key 可回收并在续接时按需重建;当前/创建/预留/在飞组始终保护;single 兼容模式继续支持单 Key `PUT` 切组
