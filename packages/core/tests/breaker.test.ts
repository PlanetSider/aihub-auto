...
describe("CircuitBreaker", () => {
	...
	test("序列化往返保留 open 状态与退避计数", () => {
		...
		const restored = CircuitBreaker.fromJSON(
	...
	test("重启保留滚动窗口,恢复前后触发语义一致", () => {
		const b = new CircuitBreaker();
		b.recordSuccess(7, T0);
		b.recordFailure(7, T0 + 100);
		const restored = CircuitBreaker.fromJSON(
			JSON.parse(JSON.stringify(b.toJSON())),
		);
		restored.recordSuccess(7, T0 + 200);
		restored.recordFailure(7, T0 + 300);
		expect(restored.isTripped(7, T0 + 300)).toBe(true);
	});

	test("fromJSON 容忍垃圾输入", () => {
		expect(() => CircuitBreaker.fromJSON(null)).not.toThrow();
		expect(() =>
			CircuitBreaker.fromJSON([{ bogus: true }, 42, "x"]),
		).not.toThrow();
	});
