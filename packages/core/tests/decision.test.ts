...
import { DEFAULT_DECISION_POLICY, decide, evaluate, idleTraffic, trafficRecency } from "../src/index.ts";
import type { DecisionPolicy, RouteState, TrafficSnapshot } from "../src/index.ts";
...
describe("trafficRecency", () => {
  ...
  test("空闲线性衰减,超 cacheIdleMs 归零", () => {
    const half: TrafficSnapshot = { lastRequestAt: NOW - policy.cacheIdleMs / 2, activeStreams: 0, requestsLast5m: 0 };
    expect(trafficRecency(half, policy, NOW)).toBeCloseTo(0.5, 5);
    const cold: TrafficSnapshot = { lastRequestAt: NOW - policy.cacheIdleMs - 1, activeStreams: 0, requestsLast5m: 0 };
...
describe("decide 基本路径", () => {
  ...
  test("当前组被淘汰 ⇒ current_invalid 直切", () => {
    const d = decide(ev(), { currentGroupId: 999 }, policy, activeTraffic(), NOW);
...
describe("粘性与缓存感知", () => {
  ...
  test("中分差 + 活跃流量 ⇒ hold_cache + pendingSwitch;空闲后同分差兑现", () => {
    ...
    // 活跃流量:держ
    const hot = decide(e, { currentGroupId: cur.stat.groupId }, policy, activeTraffic(), NOW);
    ...
    const e2 = evaluate(
      [
        stat({ groupId: 1, rateMultiplier: 0.05, avgTtftMs: 2400, lastSampleAt: new Date(later - 60_000).toISOString() }),
        stat({ groupId: 2, rateMultiplier: 0.05, avgTtftMs: 3300, lastSampleAt: new Date(later - 60_000).toISOString() }),
      ],
      opts({ now: later }),
    );
    const idle: TrafficSnapshot = { lastRequestAt: NOW, activeStreams: 0, requestsLast5m: 0 };
  ...
  test("大分差压过缓存惩罚 ⇒ 活跃期也切换", () => {
    const e = ev("economy"); // 分差远超 0.35
    const top = e.eligible[0]!;
    const cur = e.eligible[1]!;
    expect(top.score - cur.score).toBeGreaterThan(policy.stickiness + policy.cachePenaltyMax);
    const d = decide(e, { currentGroupId: cur.stat.groupId }, policy, activeTraffic(), NOW);
    expect(d.shouldSwitch).toBe(true);
  });
  ...
  test("minDwell 内不切换(dwell),但记 pendingSwitch", () => {
    ...
    expect(d.nextState.pendingSwitch?.groupId).toBe(e.eligible[0]!.stat.groupId);
  });
});
...
describe("故障转移", () => {
  ...
  test("failover 全部失败 ⇒ no_candidate", () => {
    ...
    const d = decide(e, { currentGroupId: ids[0] }, policy, idleTraffic(), NOW, {
      failover: true,
      failedGroupIds: ids,
    });
    expect(d.reason).toBe("no_candidate");
