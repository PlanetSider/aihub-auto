...
describe("硬过滤", () => {
  test("平台不匹配被排除", () => {
    const ev = evaluate([stat({ groupId: 1, platform: "anthropic" })], opts());
  ...
  test("价格区间硬边界:0.15 含,0.151 排除", () => {
    const ev = evaluate(
      [stat({ groupId: 1, rateMultiplier: 0.15 }), stat({ groupId: 2, rateMultiplier: 0.151 })],
      opts(),
    );
  ...
  test("倍率非法(负/NaN)排除", () => {
    const ev = evaluate(
      [stat({ groupId: 1, rateMultiplier: -0.1 }), stat({ groupId: 2, rateMultiplier: NaN })],
      opts(),
    );
    expect(ev.eligible).toHaveLength(0);
    expect(ev.excluded.every((e) => e.excludeReason === "invalid_rate")).toBe(true);
  });
  ...
  test("过期样本排除(>15min),未来偏差>1min 排除", () => {
    const ev = evaluate(
      [
        stat({ groupId: 1, lastSampleAt: new Date(NOW - 16 * 60_000).toISOString() }),
        stat({ groupId: 2, lastSampleAt: new Date(NOW + 2 * 60_000).toISOString() }),
        stat({ groupId: 3, lastSampleAt: new Date(NOW + 30_000).toISOString() }),
      ],
      opts(),
    );
    expect(ev.eligible.map((c) => c.stat.groupId)).toEqual([3]);
    expect(ev.excluded.map((e) => e.excludeReason).sort()).toEqual(["future_sample", "stale_sample"]);
  });

  test("无样本/非法延迟排除", () => {
    const ev = evaluate(
      [stat({ groupId: 1, sampleCount: 0 }), stat({ groupId: 2, avgTtftMs: 0 })],
      opts(),
    );
    expect(ev.eligible).toHaveLength(0);
    expect(ev.excluded.map((e) => e.excludeReason).sort()).toEqual(["invalid_latency", "no_samples"]);
  });

  test("低置信度排除:样本极少且很旧", () => {
    const ev = evaluate(
      [stat({ groupId: 1, sampleCount: 1, lastSampleAt: new Date(NOW - 14 * 60_000).toISOString() })],
      opts(),
    );
  ...
  test("本地错误率超阈直接淘汰", () => {
    const obs = new Map<number, LocalObservation>([
      [1, { groupId: 1, ewmaTtftMs: 500, errorRate: 0.8, sampleCount: 10, lastAt: NOW, confidence: 0.9 }],
    ]);
    const ev = evaluate([stat({ groupId: 1 }), stat({ groupId: 2 })], opts(), obs);
...
describe("评分与模式", () => {
  ...
  test("本地融合:高置信度本地观测拉动 blended 延迟", () => {
    const obs = new Map<number, LocalObservation>([
      [1, { groupId: 1, ewmaTtftMs: 1000, errorRate: 0, sampleCount: 20, lastAt: NOW, confidence: 1 }],
    ]);
    const ev = evaluate([stat({ groupId: 1, avgTtftMs: 5000 })], opts(), obs);
    expect(ev.eligible[0]?.blendedTtftMs).toBeCloseTo(1000, 0);
  });

  test("本地融合:α 随本地置信度线性,0.5 置信度取中点", () => {
    const obs = new Map<number, LocalObservation>([
      [1, { groupId: 1, ewmaTtftMs: 1000, errorRate: 0, sampleCount: 5, lastAt: NOW, confidence: 0.5 }],
    ]);
    const ev = evaluate([stat({ groupId: 1, avgTtftMs: 3000 })], opts(), obs);
    expect(ev.eligible[0]?.blendedTtftMs).toBeCloseTo(2000, 0);
  });
...
describe("recommendTopN", () => {
  ...
  test("−∞ 分不进推荐", () => {
    const ev = evaluate(
      [stat({ groupId: 1, rateMultiplier: 0 }), stat({ groupId: 2, rateMultiplier: 0.05 })],
      opts(),
    );
