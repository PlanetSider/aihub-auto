...
export function parseGroupStat(raw: unknown): GroupStat | undefined {
  ...
  if (platform !== "openai" && platform !== "anthropic") return undefined;
...
export class AIHubClient {
  ...
  private async request(method: string, path: string, body?: unknown): Promise<unknown> {
    ...
    try {
      ...
      throw new AIHubApiError(
        name === "TimeoutError" ? `请求超时(${this.timeoutMs}ms): ${method} ${path}` : `网络错误: ${method} ${path}`,
        0,
      );
    }

    if (res.status >= 300 && res.status < 400) {
      throw new AIHubApiError(`服务器返回重定向(${res.status}),已拒绝跟随: ${path}`, res.status);
    }
    ...
    try {
      ...
      if (!res.ok) throw new AIHubApiError(`HTTP ${res.status}: ${method} ${path}`, res.status);
      throw new AIHubApiError(`服务器返回了非 JSON 响应: ${method} ${path}`, res.status);
    }
    ...
    if (!res.ok) {
      ...
      const msg = hasEnvelope && typeof env.message === "string" ? env.message.slice(0, 200) : `HTTP ${res.status}`;
    ...
    if (hasEnvelope) {
      ...
      if (code !== "0") {
        const msg = typeof env.message === "string" ? env.message.slice(0, 200) : "业务错误";
        throw new AIHubApiError(`${msg} (${method} ${path})`, res.status, code);
      }
      return env.data;
    }
  ...
  async getUsageStats(opts: {
    ...
    if (!Number.isInteger(samples) || samples <= 0) throw new RangeError("samples 必须为正整数");
    if (opts.maxRate !== undefined && (!Number.isFinite(opts.maxRate) || opts.maxRate < 0)) {
      throw new RangeError("maxRate 必须为非负有限数");
    }
    const q = new URLSearchParams({ samples: String(samples), platform: opts.platform });
    if (opts.maxRate !== undefined) q.set("max_rate", String(opts.maxRate));
    const data = asRecord(await this.request("GET", `/api/v1/public/groups/usage-stats?${q}`));
  ...
  async login(email: string, password: string): Promise<AuthSession> {
    const data = asRecord(await this.request("POST", "/api/v1/auth/login", { email: email.trim(), password }));
    return this.toSession(data);
  }

  async refreshSession(refreshToken: string): Promise<AuthSession> {
    const data = asRecord(await this.request("POST", "/api/v1/auth/refresh", { refresh_token: refreshToken }));
  ...
  private toSession(data: Record<string, unknown>): AuthSession {
    const accessToken =
      str(data["access_token"]) || str(data["token"]) || str(data["accessToken"]);
    if (!accessToken) throw new AIHubApiError("登录响应缺少 access token", 200);
    const refreshToken = str(data["refresh_token"]) || str(data["refreshToken"]) || undefined;
  ...
  async getAvailableGroups(): Promise<GroupInfo[]> {
    ...
    return arr.map((g) => {
      ...
      return {
        id: num(r["id"]),
        name: str(r["name"]),
        platform: typeof r["platform"] === "string" ? (r["platform"] as string) : undefined,
        rateMultiplier: Number.isFinite(num(r["rate_multiplier"])) ? num(r["rate_multiplier"]) : undefined,
      };
    });
  }
  ...
  async listAllKeys(): Promise<ApiKeyInfo[]> {
    ...
    for (let page = 1; ; page++) {
      ...
      if (items.length === 0 || !Number.isFinite(pages) || page >= Math.max(pages, 1)) break;
    }
    return result;
  }

  async createKey(opts: { name: string; groupId: number }): Promise<ApiKeyInfo> {
  ...
  async updateKeyGroup(keyId: number, groupId: number): Promise<ApiKeyInfo> {
    const data = await this.request("PUT", `/api/v1/keys/${keyId}`, { group_id: groupId });
    return parseApiKey(data);
  }
