 ...
 */
export function trafficRecency(traffic: TrafficSnapshot, policy: DecisionPolicy, now: number): number {
...
export function decide(
  ...
  const switchTo = (target: ScoredCandidate, reason: Decision["reason"], threshold: number): Decision => ({
  ...
  if (!current) {
    // 当前组被硬过滤淘汰(过期/熔断/涨价出区间…)
    return switchTo(top, "current_invalid", 0);
  }
  ...
  const hold = (reason: Decision["reason"], pending?: RouteState["pendingSwitch"]): Decision => ({
    targetGroupId: current.stat.groupId,
    shouldSwitch: false,
    reason,
    currentScore: current.score,
  ...
  if (state.lastSwitchAt !== undefined && now - state.lastSwitchAt < policy.minDwellMs) {
    const pending =
      advantage > policy.stickiness ? { groupId: top.stat.groupId, since: state.pendingSwitch?.since ?? now } : undefined;
  ...
  if (advantage <= threshold) {
    ...
    return hold("hold_cache", { groupId: top.stat.groupId, since: state.pendingSwitch?.since ?? now });
  }
