...
export class CircuitBreaker {
	...
	private cooldownMs(g: GroupBreaker): number {
		...
		return Math.min(
			this.opts.baseCooldownMs * 2 ** exp,
	...
	private refresh(g: GroupBreaker, now: number): void {
		if (
			...
			now - g.openedAt >= this.cooldownMs(g)
		) {
			g.state = "half-open";
	...
	/** 未得到健康结论时释放 half-open 探针名额,供后续请求重试。 */
	releaseRequest(groupId: number, now = Date.now()): void {
		const g = this.group(groupId);
		this.refresh(g, now);
		if (g.state === "half-open") g.probeInFlight = false;
	}
	...
	recordFailure(groupId: number, now = Date.now()): void {
		...
		if (
			g.consecutiveFailures >= this.opts.consecutiveFailureThreshold ||
			rateTrip
		) {
			this.trip(g, now);
		}
	...
	static fromJSON(
		...
		for (const raw of data) {
			...
			if (r.state === "open" || r.state === "half-open" || r.state === "closed")
				g.state = r.state;
			g.consecutiveFailures =
				typeof r.consecutiveFailures === "number" ? r.consecutiveFailures : 0;
			g.window = Array.isArray(r.window)
				? r.window.filter(
						(entry): entry is { at: number; ok: boolean } =>
							typeof entry?.at === "number" && typeof entry.ok === "boolean",
					)
				: [];
