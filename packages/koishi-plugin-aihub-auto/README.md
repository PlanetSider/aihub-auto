...
查询 [AIHub](https://aihub.top) 当前最优分组推荐。群里发 `最优分组` 或 `/最优分组`,机器人回复:
...
| `scoreWindow` | `0.15` | 推荐窗口:与最优分差在窗口内才展示 |
| `strategyText` | `价格与首字延迟均衡` | 「策略:」后的文案,可自定义 |
| `downloadUrl` | 应用 Releases | 自动路由应用下载链接 |
| `template` | 见下 | 回复模板,变量 `{strategy}` `{items}` `{download}` |
...
- `/最优分组`(经指令系统,支持你配置的全局 prefix 与 help)
- 裸文本 `最优分组`(中间件精确匹配,@机器人后跟触发词也可)

两种方式同一消息只回一次;冷却期内重复触发静默。
