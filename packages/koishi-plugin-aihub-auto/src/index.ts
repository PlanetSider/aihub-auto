...
export const Config: Schema<Config> = Schema.object({
  rules: Schema.array(Schema.string())
    .default(["onebot:*"])
    .description("生效范围,每项 `平台:群号`,支持 `*` 通配。如 `onebot:1059338666`、`onebot:*`、`*:*`。"),
  baseUrl: Schema.string()
    .default("https://aihub.top")
    .description("AIHub 站点地址(usage-stats 为 AIHub 自有接口,不兼容其他 sub2api 站)。"),
  mode: Schema.union([
  ...
  maxRate: Schema.number().min(0).default(0.15).description("最大倍率(硬约束,超过不推荐)。"),
  samples: Schema.number().min(1).max(500).default(100).description("每组统计样本条数。"),
  ...
  cacheTtlMs: Schema.number().min(0).default(30_000).description("推荐结果缓存时长(毫秒)。"),
  cooldownMs: Schema.number().min(0).default(10_000).description("每群冷却(毫秒),期间重复触发不响应。"),
...
export function apply(ctx: Context, config: Config): void {
  ...
  function inScope(platform: string, guildId: string | undefined, isDirect: boolean): boolean {
  ...
  function underCooldown(scopeKey: string): boolean {
    ...
    if (last !== undefined && Date.now() - last < config.cooldownMs) return true;
    cooldown.set(scopeKey, Date.now());
    return false;
  }

  async function respond(messageId: string | undefined, scopeKey: string): Promise<string | undefined> {
    if (messageId) {
  ...
  function scopeKey(platform: string, guildId: string | undefined, userId: string | undefined): string {
  ...
  ctx
    ...
    .action(async ({ session }) => {
      if (!session) return;
      if (!inScope(session.platform, session.guildId, session.isDirect ?? !session.guildId)) return;
      return respond(session.messageId, scopeKey(session.platform, session.guildId, session.userId));
    });
  ...
  ctx.middleware(async (session, next) => {
    ...
    if (!inScope(session.platform, session.guildId, session.isDirect ?? !session.guildId)) return next();
    const reply = await respond(session.messageId, scopeKey(session.platform, session.guildId, session.userId));
