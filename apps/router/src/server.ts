...
export async function handleControl(req: Request, url: URL, deps: ServerDeps): Promise<Response> {
  ...
  if (path === "/ctl/config" && req.method === "POST") {
    ...
    const allowed = ["mode", "keyMode", "priceBand", "blacklist", "pollIntervalMs", "poolMaxGroups", "samples"];
    ...
    if (!parsed.success) {
      return json({ error: `配置校验失败:${parsed.error.issues.map((i) => i.message).join("; ")}` }, 400);
    }
  ...
  if (path === "/ctl/login" && req.method === "POST") {
    ...
    try {
      ...
      return json({ error: err instanceof Error ? err.message : "登录失败" }, 400);
    }
  }
