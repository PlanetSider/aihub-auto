...
export class MockAIHub {
  ...
  private async handle(req: Request): Promise<Response> {
    ...
    if (path === "/api/v1/auth/login" && req.method === "POST") {
      this.loginCalls++;
      return this.envelope({ access_token: "mock-at", refresh_token: "mock-rt", expires_in: 3600 });
    }
    if (path === "/api/v1/auth/refresh" && req.method === "POST") {
      this.refreshCalls++;
      this.expireToken = false;
      return this.envelope({ access_token: "mock-at-2", refresh_token: "mock-rt-2", expires_in: 3600 });
    }
    if (path === "/api/v1/auth/me") {
      if (this.expireToken) return this.json({ code: 1, message: "unauthorized" }, 401);
    ...
    if (path === "/api/v1/keys" && req.method === "POST") {
      ...
      const key: MockKey = { id, name: body.name, key: `sk-mock-${id}`, group_id: body.group_id };
    ...
    if (path.startsWith("/v1/")) {
      ...
      if (b.status && b.status >= 400) {
        return this.json({ error: { message: `mock error group ${groupId}` } }, b.status);
      }
      if (b.sse) {
        ...
        const stream = new ReadableStream<Uint8Array>({
          async start(c) {
            ...
            c.enqueue(enc.encode(`data: {"choices":[{"delta":{"content":"来自组 ${groupId}"}}]}\n\n`));
            await Bun.sleep(10);
            if (breakMid) {
      ...
      return this.json({ id: "cmpl-1", group: groupId, content: b.body ?? `response from group ${groupId}` });
    }
...
export function makeStat(partial: Partial<GroupStat> & { groupId: number }): GroupStat {
  return {
