...
describe("缓存感知端到端", () => {
  test("持续流量中小幅更优 ⇒ 不切(hold_cache);流量停止后 ⇒ pending_realized", async () => {
    h = createHarness({
      configPatch: {
        keyMode: "pool",
        decision: { stickiness: 0.1, cachePenaltyMax: 0.25, cacheIdleMs: 1_000, minDwellMs: 0 },
      },
    });
    // 先路由到组 2
    h.mock.stats = [makeStat({ groupId: 2, rateMultiplier: 0.05, avgTtftMs: 3000 })];
    await h.daemon.runOnce();
    expect(h.state.currentGroupId).toBe(2);
  ...
  test("活跃流量中大幅优势 ⇒ 当场切换", async () => {
    h = createHarness({
      configPatch: { keyMode: "pool", decision: { stickiness: 0.1, cachePenaltyMax: 0.25, cacheIdleMs: 300_000, minDwellMs: 0 } },
    });
    h.mock.stats = [makeStat({ groupId: 2, rateMultiplier: 0.12, avgTtftMs: 6000 })];
    await h.daemon.runOnce();
    h.mock.stats = [
      makeStat({ groupId: 1, rateMultiplier: 0.02, avgTtftMs: 1000 }),
...
describe("状态持久化与恢复", () => {
  test("breaker/observations 序列化进 state,重建后语义保留", async () => {
    ...
    // 模拟重启:从序列化数据重建
    const b2 = CircuitBreaker.fromJSON(JSON.parse(JSON.stringify(h.state.breaker)));
    const o2 = LocalObservationStore.fromJSON(JSON.parse(JSON.stringify(h.state.observations)));
...
describe("控制台 API", () => {
  test("status/config/route-once/login 全链路", async () => {
    ...
    const route = (await routeRes.json()) as { shouldSwitch: boolean; targetGroupId: number };
    ...
    const status = (await statusRes.json()) as {
      ...
      candidates: { groupId: number; excluded: boolean; excludeReason?: string }[];
      hasToken: boolean;
    };
  ...
  test("uiPassword 配置后:无口令 401,带口令通过;/healthz 与 /ui 开放", async () => {
    ...
    expect(
      (await fetch(`${base}/ctl/status`, { headers: { "x-ui-password": "console-pass-123" } })).status,
    ).toBe(200);
